...
Une fois connecté à **Lumina Studio** (`http://localhost:7080` ou `https://lumina.fr`), vous accédez à votre tableau de bord central :
...
2. **Module Carnet de route (`hasCarnet`)** :
...
| :--- | :--- | :--- |
| **Studio (Manager)** | `http://localhost:7080` | `https://lumina.fr` |
...
2. Spécifiez le type (Boîtier ou Objectif), la marque, le modèle, le format supporté (*135*, *120*, *plan-film 4x5/9x12*) et le numéro de série facultatif.
...
2. Renseignez la marque (ex: *Kodak*), le type (ex: *Tri-X 400*), la sensibilité nominale (ISO 400), la sensibilité utilisée (ISO 800 si poussé) et le format (*135*, *120*, *plan-film*).
